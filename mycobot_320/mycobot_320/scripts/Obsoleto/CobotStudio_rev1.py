...
class SimManager:
    ...
    def MoveJ(self, wobj, robtarget, tool: SE3 | None = SE3(), wobj_name='wobj1', robt_name='robtarget'):
        ...
        # q_gripper = self.q_current[6:]
        # q_start = self.q_current.copy()[:7]
        # q_start = to_array(self.q_current)  # vector 7
        ...
        # cobot_tb.genTrJoint(traj_q, 0 * np.ones(3))
        qtraj_a = cobot_tb.q_ref[::10]
        # qtraj_a_full = [np.concatenate([q[:6], q_gripper]) for q in qtraj_a]
        # qtraj_a_full = [np.concatenate([q[:6], [gripper_val]]) for q in qtraj_a]
        qtraj_a_full = [np.concatenate([q, [gripper_val]]) for q in qtraj_a]

        self._send_move(qtraj_a_full, robt_name, wobj_name)

    def MoveC(self, wobj, robtarget, tool: SE3 | None = None, wobj_name='wobj1', robt_name='robtarget'):
        self.node_tf.add_wobj(wobj, wobj_name)
        self.node_tf.add_robt(robtarget.pose, robt_name, wobj_name)
        ...
        cobot_tb.genTrCart([pose_start, pose_goal, pose_goal], 0*np.ones(3), conf = robtarget.config)  # si tu función acepta otros parámetros, los podés poner acá
    ...
    def VerPose(self, wobj, robtarget, tool: SE3 | None = SE3(), wobj_name='wobj1', robt_name='robtarget'):
        ...
        self._send_pose(q, robt_name, wobj_name)
    ...
    def _send_pose(self, q_pos, robt_name = 'robt', wobj_name = 'wobj'):
        ...
        pub_thread.start()
        pub_thread.join()
        self.q_current = q_pos.tolist() + [0.0]*6
    ...
    def mover_pinza(self, apertura: float):
        """
        ...
        # Solo interesa la posición del robot hasta el gripper_controller inclusive
        joint_names_gripper = [
        'joint2_to_joint1', 'joint3_to_joint2', 'joint4_to_joint3',
        'joint5_to_joint4', 'joint6_to_joint5', 'joint6output_to_joint6',
        'gripper_controller']

        # Copiar el estado actual y modificar solo gripper_controller
        q_actual = to_array(self.q_current)
        q_gripper = q_actual[:7]
        # Mapear apertura [0,100] a rango [-0.7, 0.3]
        q_actual[6] = apertura / 100 - 0.7
        print(f'q_actual = {q_actual}')
        print(f'type q_actual = {type(q_actual)}')
        print(f'q_gripper = {q_gripper}')
        # q_publish = q_gripper.tolist() + [0.0]*5
        q_publish = q_gripper
        # self.node_joint.publish_q(q_actual, joint_names)
        msg = JointState()
        msg.name = joint_names
...
class MyCobotController:
    ...
    def MoveJ(self, robt, speed: int, tool: SE3 | None = None, wobj: SE3 | None = None):
        ...
        q_pose = self.cobot_tb.ikine(pose_calc, robt.config)[0]

        # Enviar al robot (la API espera grados)
        self.mc.send_angles(np.degrees(q_pose).tolist(), speed)

    def MoveC(self, robt, speed: int, tool: SE3 | None = None, wobj: SE3 | None = None):
        pose_calc = wobj * robt.pose * tool.inv()

        # Convertir a formato [x, y, z, rx, ry, rz] en grados.
        pose = list(pose_calc.t) + list(pose_calc.rpy(order='zyx', unit='deg'))

        self.mc.send_coords(pose, speed, 1)
    ...
    def GripperState(self, mov: int, spd: int = 30):
        """
...
def teach_wobj(q_poses, tool : SE3, z_aux = 25):
    """
    Enseña un workobject a partir de tres puntos medidos manualmente con el robot.
    ...
    #     else:
    #         t_punto = 10
    #     print(f"Tenés {t_punto} segundos para ajustar la herramienta en la posición y orientación deseada.")
    #     for i in range(t_punto, 0, -1):
    #         print(f"Tiempo restante antes de activar motores: {i} segundos...", end='\r')
    #         time.sleep(1)
    ...
    #     mediciones_np = np.array(mediciones)
    #     pose_promedio = np.mean(mediciones_np, axis=0).tolist()
    #     print(f"Punto {i+1}: {pose_promedio}")
    #     puntos.append(np.array(pose_promedio))
    puntos = np.zeros((3,3))

    for i in range (3):
        puntos_sc = (cobot_tb.fkine(np.radians(q_poses[i])) * tool * SE3(0, 0, 25)).t
        # correccion = np.array([0, -abs(z_aux), 0])
