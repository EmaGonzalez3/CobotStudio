            ...
            try:
                ...
                while True:
                    try:
                        ...
                        # Comando especial para reiniciar el puerto serie remotamente
                        if data == b'reset_serial':
                            self.logger.info("Comando de reinicio de puerto serie recibido.")
                            try:
                                self.mc.close()
                                time.sleep(0.5)  # Pausa para asegurar el cierre
                                self.mc.open()
                                self.logger.info("Puerto serie reiniciado exitosamente.")
                                conn.sendall(b'Serial port reset OK')
                            except Exception as e:
                                self.logger.error(f"Fallo al reiniciar el puerto serie: {e}")
                                conn.sendall(b'Serial port reset FAILED')
