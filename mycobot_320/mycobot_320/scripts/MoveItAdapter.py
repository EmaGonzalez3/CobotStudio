...
class MoveItAdapter(Node):
    ...
    def save_last_planned_trajectory(self, out_filename: str, as_python: bool = True) -> bool:
        """
        Guarda la última DisplayTrajectory como:
          - .py (lista de listas en formato Python) si as_python True
          - .json si as_python False
        Extrae los joint positions de cada JointTrajectoryPoint.
        """
        msg = self.last_planned_trajectory()
        if msg is None:
            self.get_logger().warning("No display_planned_path recibida para guardar.")
            return False

        # extraer la primer trajectory (usualmente move_group publica una lista; nos quedamos con la primera)
        if not msg.trajectory:
            self.get_logger().warning("DisplayTrajectory vacío.")
            return False
        ...
        q_list = []
        for p in points:
            q_list.append(list(p.positions))

        # Guardar en un archivo con el nombre y formato elegidos.
        base, ext = os.path.splitext(out_filename)
        if as_python:
            if ext == "":
                out_filename = out_filename + ".py"
            try:
                with open(out_filename, "w") as f:
                    f.write("# Autogenerated: lista de posiciones (radianes)\n")
                    f.write("TRAJ = [\n")
                    for q in q_list:
                        f.write(f"    {q},\n")
                    f.write("]\n")
                    f.write(f"JOINT_NAMES = {joint_names}\n")
                self.get_logger().info(f"Guardado trajectory -> {out_filename} (len={len(q_list)})")
                return True
            except Exception as e:
            ...
            try:
                with open(out_filename, "w") as f:
                    json.dump({"joint_names": joint_names, "traj": q_list}, f, indent=2)
                self.get_logger().info(f"Guardado trajectory -> {out_filename} (len={len(q_list)})")
                return True
            except Exception as e:
                self.get_logger().error(f"Error guardando archivo: {e}")
