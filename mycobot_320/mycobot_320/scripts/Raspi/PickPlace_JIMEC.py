from CobotStudio_rev5 import RobTarget, BaseRobotController, MyCobotController, ROS_OK
...
if ROS_OK:
    from CobotStudio_rev5 import SimManager
...
def get_robot(mode="sim", **kwargs) -> BaseRobotController:
    if mode == "sim" and ROS_OK:
        return SimManager()
    elif mode == "real":
        return MyCobotController(**kwargs)
    elif mode == "sim" and not ROS_OK:
        raise RuntimeError("Se intentó iniciar el modo 'sim' pero ROS2 no está disponible.")
    else:
...
def main(robot: BaseRobotController, devolver = True):
    ...
    for i in range (3):
        ...
        robot.MoveCTesting(pick, 30, pinza, wobj, 5)
        time.sleep(1)
        robot.GripperState(0, 30) # Cerrar pinza
        time.sleep(2) # Hay que darle tiempo a que cierre
        robot.MoveCTesting(pick.offset(0, 0, 30), 30, pinza_caja, wobj)
...
if __name__ == "__main__":
    if ROS_OK:
        print(">>> ROS2 detectado. Iniciando en modo SIMULACIÓN. <<<")
        execution_mode = "sim"
    else:
        print(">>> ROS2 no detectado. Iniciando en modo REAL. <<<")
        execution_mode = "real"
    # robot_controller = get_robot("sim")
    # robot_controller = get_robot("real") # Cobot real
    robot_controller = get_robot(execution_mode)
