...
class MycobotServer(object):
    ...
    def __init__(self, host, port, serial_num = "/dev/ttyAMA0", baud = 1000000):
        ...
        # Serial a 115200 (para power_on)
        self.mc_low = serial.Serial(serial_num, 115200, timeout=0.1)

        # Serial a 1000000 (para movimientos)
        self.mc_high = serial.Serial(serial_num, 1000000, timeout=0.1)
                ...
                while True:
                    try:
                        ...
                        if self.mc.isOpen() == False:
                            ...
                            # POWER_ON → usar mc_low
                            if command[3] == 0x10:
                                self.logger.info("POWER_ON command, sending via 115200")
                                self.write(self.mc_low, command)
                            # si esperamos respuesta:
                            # res = self.read(self.mc_low, command)
                            # conn.sendall(res)
                            else:
                                # resto de comandos → usar mc_high
                                self.logger.info("Other command, sending via 1000000")
                                self.write(self.mc_high, command)
                                if command[3] in has_return:
                                    res = self.read(self.mc_high, command)
                                    conn.sendall(res)
    ...
    def write(self, mc, command):
        mc.write(bytes(command))
        mc.flush()

    def read(self, mc, command):
        datas = b""
        data_len = -1
        k = 0
        pre = 0
...
if __name__ == "__main__":
    ...
    MycobotServer(HOST, PORT, "/dev/ttyAMA0", 115200)
