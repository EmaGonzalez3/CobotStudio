...
class SimManager:
    def __init__(self):
        ...
        self.node_obj = ObjectManager(self.node_joint)
    ...
    def mover_pinza(self, apertura: float, tool: SE3 | None = SE3()):
        """
...
def teach_wobj(q_poses, tool : SE3, z_aux = 25):
    ...
    for i in range (3):
        # puntos_sc = ((cobot_tb.fkine(np.radians(q_poses[i])) )* tool).t # Ok si se agrega el offset en la tool.
        puntos_sc = ((cobot_tb.fkine(np.radians(q_poses[i])) )* tool* SE3(0, 0, abs(z_aux))).t
        # correccion = np.array([0, -abs(z_aux), 0])
