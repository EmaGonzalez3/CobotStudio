...
class myCobot320(DHRobotGT):
    def __init__(self,*args, metros=False, rotar_base=False, **kwargs):
        ...
        eje1 = rtb.RevoluteDH(alpha=-np.pi/2,a=0,d=173.87*esc,offset=0,qlim=[-170*np.pi/180,170*np.pi/180])
        eje2 = rtb.RevoluteDH(alpha=0,a=134.96*esc,d=0,offset=-np.pi/2,qlim=[-120*np.pi/180,120*np.pi/180])
        eje3 = rtb.RevoluteDH(alpha=0,a=120*esc,d=0,offset=0,qlim=[-148*np.pi/180,148*np.pi/180]) 
        eje4 = rtb.RevoluteDH(alpha=np.pi/2,a=0,d=88.74*esc,offset=np.pi/2,qlim=[-120*np.pi/180,135*np.pi/180])
        eje5 = rtb.RevoluteDH(alpha=-np.pi/2,a=0,d=95*esc,offset=0,qlim=[-169*np.pi/180,169*np.pi/180])    
        eje6 = rtb.RevoluteDH(alpha=0,a=0,d=65.5*esc,offset=0,qlim=[-180*np.pi/180,180*np.pi/180])
